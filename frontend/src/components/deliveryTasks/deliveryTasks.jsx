...
class DeliveryTasks extends Component {
  ...
  getOrders = async () => {
    try {
      let orders = await axios.get(backendurl + 'orders/' + localStorage.getItem("userId") + '/pickedUp');
  ...
  markOrderDelivered = async (orderId) => {
    try {
      let orderResponse = await axios.post(backendurl + 'order/markdelivered/' + orderId);
                    ...
                    ordersList.map((order, rowIndex) => {
                      return (
                        <tr key={rowIndex}>
                          ...
                          <td className="text-center">{order.pooler && order.pooler.nickName}</td>
                          <td className="text-center">{order.price}</td>
                          <td className="text-center">{order.status}</td>
                          <td className="text-center" colSpan={2}>
                            {order.pooler && ('Street - ' +  order.pooler.address.street + ',' +
                              'City - ' + order.pooler.address.city + ',' +
                              'State -' + order.pooler.address.state + ',' +
